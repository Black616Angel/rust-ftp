...
impl FtpStream {
    ...
    pub fn into_secure(mut self, ssl_context: SslContext) -> Result<FtpStream> {
        ...
        let ssl_cfg = (Ssl::new(&ssl_context).map_err(|e| FtpError::SecureError(e.description().to_owned())))?;
        let stream = (ssl_cfg.connect(self.reader.into_inner().into_tcp_stream()).map_err(|e| FtpError::SecureError(e.description().to_owned())))?;
    ...
    pub fn retr_with_file<F, T>(&mut self, filename: &str, file: &mut File, reader: F) -> Result<T>
    where F: Fn(&mut Read, &mut File) -> Result<T> {
        let retr_command = format!("RETR {}\r\n", filename);
        {
            let mut data_stream = BufReader::new((self.data_command(&retr_command))?);
            self.read_response_in(&[status::ABOUT_TO_SEND, status::ALREADY_OPEN])
                .and_then(|_| reader(&mut data_stream, file))
        }.and_then(|res|
            self.read_response_in(&[status::CLOSING_DATA_CONNECTION,status::REQUESTED_FILE_ACTION_OK]).map(|_| res))
    }
    ...
    pub fn size(&mut self, pathname: &str) -> Result<Option<usize>> {
        ...
        match SIZE_RE.captures(&content) {
            Some(caps) => {
			let parsed = caps[1].parse();
			if parsed.is_ok() {
				Ok( Some(parsed.unwrap()))
			} else
			{
				Ok(Some(usize::MAX))
			}
